...
class PhotoEditingViewController: UIViewController, PHContentEditingController {
    ...
    func showFilter() {
        let alertController = UIAlertController(title: "필터", message: "어떤 필터를 적용하시겠습니까?", preferredStyle: UIAlertControllerStyle.alert)
        
        let actionMono = UIAlertAction(title: "Mono", style: UIAlertActionStyle.default, handler: {(action: UIAlertAction) in
            self.selectedFilter = "CIPhotoEffectMono"
            self.imageView.image = self.applyFilter(self.image!)
        })

        
        let actionInstant = UIAlertAction(title: "Instant", style: UIAlertActionStyle.default, handler: {(action: UIAlertAction) in
        ...
        DispatchQueue.global().async {
            ...
            if let url = self.input!.fullSizeImageURL {
                ...
                if (((try? renderedData?.write(to: output.renderedContentURL, options: [.atomic])) != nil) != nil) {
                    let archiveData = NSKeyedArchiver.archivedData(withRootObject: self.selectedFilter!)
